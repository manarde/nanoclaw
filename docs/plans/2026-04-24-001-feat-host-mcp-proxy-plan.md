...
- **Handler ordering (new `host_mcp_query` case):**
  ...
  8. **Spawn:** `argv = ['-p', '--dangerously-skip-permissions', '--allowed-tools', scopePrefixes + ',mcp__nanoclaw_host__host_mcp_reply', \`/host-mcp-agent ${scope} ${sourceGroup} ${requestId}\`]`. The extra allowed tool is the reply primitive from U8. `deps.spawnHostClaude(argv, { cwd, stdio: ['ignore','pipe','pipe'] })`
...
- **Unchanged invariants:**
  ...
  - Per-group IPC namespace = auth boundary for the **container** (no change; container can only write into its own group's IPC subtree). The **host-side** claude session has full filesystem privileges at the OS level but is sandboxed at the MCP-tool level: `--allowed-tools` grants only the scope's PB prefix plus `host_mcp_reply`. Bash, Write, Edit are revoked. The `host_mcp_reply` tool (U8) bakes in `sourceGroup` + `chatJid` so the agent cannot write to the wrong group even if prompt-injected
...
- **TCB assumption reduced (not eliminated):** with U8's `host_mcp_reply` primitive and `--allowed-tools` revoking Bash/Write/Edit from the host session, the remaining TCB trust is that the MCP protocol itself enforces the allowlist (it does, at the Claude SDK layer). A sufficiently sophisticated prompt injection of an agent with **only** PitchBook tools + `host_mcp_reply` can do exactly two things: (1) query PitchBook under Matt's auth, (2) write a reply to Matt's source chat. It cannot reach `data/ipc/*/tasks/`, other groups, or the filesystem at large. This is a substantially tighter trust boundary than SKILL.md instruction alone
...
|------|-----------|--------|------------|
| **Prompt-injected question pivots host agent to non-PB MCPs** (e.g., Gmail exfiltration once Gmail MCP is configured) | Low (today — no second MCP) / Medium (post-Gmail) | **Critical** | **Primary:** `--allowed-tools` argv on spawn restricts the host session to only `mcp__claude_ai_PitchBook_Premium__*` + `mcp__nanoclaw_host__host_mcp_reply`; Bash/Write/Edit revoked. **Secondary:** SKILL.md scope-guardrail table. The argv constraint is enforced by `claude -p` itself |
